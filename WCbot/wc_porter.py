...
from MultiBot.utils import image_filename, format_filename
from MultiBot.distributor import Distributor, DistributorCron
from MultiBot.server_config import WECHATY_ENV
...
class MyBot(Wechaty):
    ...
    async def on_message(self, msg: Message):
        ...
        if msg.type() == MessageType.MESSAGE_TYPE_TEXT:
            ...
            # filter url & email address
            match = re.match(r'<a target="_blank" .*>(.*)</a>', text)
            if match:
                text = match.group(1)
            if not request.echo:
                request.msg = text
