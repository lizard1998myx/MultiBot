...
class SubNaocExpSession(Session):
    def __init__(self, user_id):
        Session.__init__(self, user_id=user_id)
        self._max_delta = 60
        self.session_type = '国台自动填报 - beta'
        self.strict_commands = ['subnaoc', '国台填报', '国台填报ex']
        self.description = '登记自动疫情填报（国台版），半自动获取cookie并提交。实验型。'
    ...
    def handle(self, request):
        if self.is_first_time:
            ...
            if request.msg != '准备好了':
                ...
                return [ResponseMsg(f'【{self.session_type}】请在60秒内扫码、在手机上点击确认，然后回复任意消息继续。'),
                        ResponseImg(file=img_file)]
        elif self.is_third_time:
            self.is_third_time = False
